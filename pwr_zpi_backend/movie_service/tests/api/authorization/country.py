...
from movie_service.models import Country

from . import AbstractApiModelAuthTests


class ApiCountryAuthTests(AbstractApiModelAuthTests):
    rev_urls = {
        'list': 'country-list',
    ...
    def helper_test_create_access(self, user, expected_status):
        ...
        response = self.client.post(
            reverse(self.rev_urls['list']), self.data['country_01'], format='json'
        )
    ...
    def helper_test_delete_access(self, user, expected_status):
        self.client.force_authenticate(user)
        new_country = Country.objects.create(**self.data['country_01'])
        response = self.client.delete(
            reverse(self.rev_urls['detail'], kwargs={'pk': new_country.pk}), format='json'
        )
        self.assertEqual(response.status_code, expected_status)
    ...
    def helper_test_update_access(self, user, expected_status):
        self.client.force_authenticate(user)
        new_country = Country.objects.create(**self.data['country_01'])
        modified_country = self.data['country_01'].copy()
        modified_country['name'] = self.data['country_02']['name']
        response = self.client.put(
            reverse(self.rev_urls['detail'], kwargs={'pk': new_country.pk}),
            data=modified_country, format='json'
        )
        self.assertEqual(response.status_code, expected_status)
    ...
    def helper_test_partial_update_access(self, user, expected_status):
        ...
        response = self.client.patch(
            reverse(self.rev_urls['detail'], kwargs={'pk': new_country.pk}),
            data={'name': self.data['country_02']['name']}, format='json'
        )
        self.assertEqual(response.status_code, expected_status)
