...
from movie_service.models import RequestLog, TitleRequest, TitleSubmission

from . import AbstractApiModelAuthTests


class ApiRequestLogAuthTests(AbstractApiModelAuthTests):
    rev_urls = {
        'list': 'request_log-list',
        'detail': 'request_log-detail',
    }
    data = {
        'object': {
            'details': 'Mama, just killed a man',
        },
        'object_02': {
            'details': "Sweet dreams are made of this",
        },
        'title_request_01': {
    ...
    def setUpTestData(cls):
        ...
        cls.tr = cls._create_tr()

    @classmethod
    def _create_tr_submission(cls, tr):
        data = cls.data['title_request_01']['title_submission'].copy()
        data['title_request'] = tr
        return TitleSubmission.objects.create(**data)

    @classmethod
    def _create_tr(cls):
        data = cls.data['title_request_01'].copy()
        data['user'] = cls.user
        del data['title_submission']
        tr = TitleRequest.objects.create(**data)
        cls._create_tr_submission(tr)
        return tr

    def _create_log(self):
        data = self.data['object'].copy()
        data['moderator'] = self.moderator
        data['title_request'] = self.tr
        return RequestLog.objects.create(**data)
