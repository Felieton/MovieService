        ...
        <v-card>
          <v-row>
            <v-col class="col-10">
              ...
              <div v-if="item.current_title" class="d-flex text-break text-subtitle-1 px-4 pt-4">
                <router-link
                    class="link-hover"
                    :to="{ name: 'titles_single', params: { id: item.current_title.id }}"
                >
                ...
                >
                  ...
                  >
                    <v-btn
                        ...
                        v-bind="attrs"
                        v-on="on"
                        :to="{ name: 'mod_queue_title_one', params: { id: item.id } }"
                    >
  ...
  methods: {
    ...
    async acceptRequest (request) {
      try {
        ...
        this.showSnackSuccess('Title request accepted')
      } catch (err) {
        if (err.response) {
    ...
    async rejectRequest (request) {
      try {
        ...
        this.showSnackSuccess('Title request rejected')
      } catch (err) {
        if (err.response) {
          const errorMsg = this.formatResponseErrors(err.response)
